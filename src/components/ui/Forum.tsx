...
import { MessageSquare, ThumbsUp, Send, ChevronDown, ChevronUp, User } from "lucide-react";
      ...
      {post.tags && post.tags.length > 0 && (
        <div className="mt-3 flex flex-wrap gap-1.5">
          {post.tags.map((tag) => (
            <span
              key={tag}
              className="rounded-full bg-cyan-500/10 px-2.5 py-0.5 text-[10px] font-medium text-cyan-400"
            >
              {tag}
            </span>
          ))}
...
export function Forum({
  ...
  className = "",
  title = "Forum",
  emptyMessage = "No posts yet. Be the first to start a discussion!",
